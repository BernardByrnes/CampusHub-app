...
export type SportsAuditMutationAppendInput = Readonly<{
  tenantId: string;
  actorMembershipId: string;
  ...
  eventFacts: SportsAuditEventFacts;
}>;

export type FixtureAuditMutationAppendInput = Readonly<{
  tenantId: string;
  actorMembershipId: string;
  ...
  eventFacts: FixtureAuditEventFacts;
}>;

export type ResultAuditMutationAppendInput = Readonly<{
  tenantId: string;
  actorMembershipId: string;
  resourceId: string;
  resourceVersion: number;
...
export type OrganiserAuditMutationAppendInput = Readonly<{
  tenantId: string;
  actorMembershipId: string;
  resourceId: string;
  resourceVersion: number;
  ...
  eventFacts: OrganiserAuditEventFacts;
}>;
...
export class DrizzleAuditEventRepository {
  ...
  /**
   * Appends one minimized Sports configuration event to the same transaction
   * as its resource mutation. The closed event/resource pairs are validated
   * again here so callers cannot turn the audit repository into a generic
   * event-writing escape hatch.
   */
  public async appendSportsMutationInTransaction(
    transaction: AuditEventTransactionDatabase,
    input: SportsAuditMutationAppendInput,
  ): Promise<AuditEvent> {
    const eventFacts = normalizeSportsAuditEventFacts(
      input.eventFacts,
      input.eventType,
      input.resourceType,
    );
    if (
      ...
      !(input.occurredAt instanceof Date) ||
      Number.isNaN(input.occurredAt.getTime()) ||
      !SPORTS_AUDIT_EVENT_TYPES.includes(input.eventType) ||
      eventFacts === null
    ) {
      throw new Error("Invalid Sports audit input.");
    }
    ...
    if (
      ...
      persisted.resourceType !== input.resourceType
    ) {
      throw new Error("Sports audit append returned an invalid persisted row.");
    }
    return persisted;
  }

  /** Appends a closed Fixture lifecycle event to the same transaction as the
   * Fixture mutation. The append-only repository deliberately exposes no
   * update, delete, truncate, or generic event-writing operation. */
  public async appendFixtureMutationInTransaction(
    transaction: AuditEventTransactionDatabase,
    input: FixtureAuditMutationAppendInput,
  ): Promise<AuditEvent> {
    const eventFacts = normalizeFixtureAuditEventFacts(
      input.eventFacts,
      input.eventType,
    );
    if (
      ...
      !(input.occurredAt instanceof Date) ||
      Number.isNaN(input.occurredAt.getTime()) ||
      !FIXTURE_AUDIT_EVENT_TYPES.includes(input.eventType) ||
      eventFacts === null
    ) {
      throw new Error("Invalid Fixture audit input.");
    }
    ...
    if (
      ...
      persisted.currentHash !== currentHash ||
      persisted.eventType !== input.eventType ||
      persisted.resourceType !== "fixture"
    ) {
      throw new Error("Fixture audit append returned an invalid persisted row.");
    }
    return persisted;
  }

  /** Appends one closed Result lifecycle event atomically with the Result
   * mutation. Result revisions themselves are append-only database rows; the
   * audit chain records the aggregate transition without exposing actor IDs to
   * student read models. */
  public async appendResultMutationInTransaction(
    transaction: AuditEventTransactionDatabase,
    input: ResultAuditMutationAppendInput,
  ): Promise<AuditEvent> {
    const eventFacts = normalizeResultAuditEventFacts(
      input.eventFacts,
      input.eventType,
    );
    if (
      ...
      !(input.occurredAt instanceof Date) ||
      Number.isNaN(input.occurredAt.getTime()) ||
      !RESULT_AUDIT_EVENT_TYPES.includes(input.eventType) ||
      eventFacts === null
    ) {
      throw new Error("Invalid Result audit input.");
    }
  ...
  /** Appends one minimized Organiser lifecycle event atomically with the
   * Organiser mutation. The append-only repository exposes no generic writer. */
  public async appendOrganiserMutationInTransaction(
    transaction: AuditEventTransactionDatabase,
    input: OrganiserAuditMutationAppendInput,
  ): Promise<AuditEvent> {
    const eventFacts = normalizeOrganiserAuditEventFacts(
      input.eventFacts,
      input.eventType,
    );
    if (
      ...
      !(input.occurredAt instanceof Date) ||
      Number.isNaN(input.occurredAt.getTime()) ||
      !ORGANISER_AUDIT_EVENT_TYPES.includes(input.eventType) ||
      eventFacts === null
    ) {
      throw new Error("Invalid Organiser audit input.");
    }
    ...
    if (
      ...
      persisted.currentHash !== currentHash ||
      persisted.eventType !== input.eventType ||
      persisted.resourceType !== "organiser"
    ) {
      throw new Error("Organiser audit append returned an invalid persisted row.");
    }
    return persisted;
  }
